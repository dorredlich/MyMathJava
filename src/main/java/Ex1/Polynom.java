...
public class Polynom implements Polynom_able {
    ...
    private int getPowerForString(String s) throws Exception {
        ...
        if (!isNumeric(s.substring(xIndex + 1, s.length())))
            throw new Exception("Invalid input of : " + s.substring(xIndex + 1, s.length()));
    ...
    /**
     * Multiplying the polynom by the m1 monom
     *
     * @param m1 the monom we multiply by
     */
    @Override
    public void multiply(Monom m1) {
        ...
        while (itr.hasNext()) {
            ...
            Monom x = new Monom(a.multiply(m1));
            temp.add(x);
        }
        temp.M.sort(cmp);
        M = temp.M;
    ...
    /**
     * Constructor which gets String and turn it to function by the help of init the polynom
     *
     */
    @Override
    ...
    /**
     * boolean function that check if object is kind of polynom and check if two polynom are equals.
     *
     * @param: obj: represent the Object
     */
    public boolean equals(Object obj) {
        Iterator<Monom> itr1 = M.iterator();
        if (obj != null && obj instanceof Polynom) {//if the Object is kind of polynom do downcasting to this object.
            Iterator<Monom> itr2 = ((Polynom) obj).iteretor();
            if (this.isZero() && ((Polynom) obj).isZero()) return true;
            while (itr1.hasNext() && itr2.hasNext()) {
                Monom m1 = itr1.next();
                Monom m2 = itr2.next();
                if ((m1.get_coefficient() != m2.get_coefficient()) || (m1.get_power() != m2.get_power()))
